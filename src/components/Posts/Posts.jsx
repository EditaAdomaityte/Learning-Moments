...
import {getAllPosts,getAllTopics,} from "../../services/postServices/postServices";
import "./PostList.css";
...
// Filter posts based on search term and selected topic
  useEffect(() => {
    ...
    // Apply topic filter
      if(selectedTopic !=="Select a Topic"){                        //we check if a topic is actually selected,This means we only filter if the user has picked a real topic (not the default "Select a Topic" option
        filtered=filtered.filter((post)=>{                          //we filter the posts, starts the filtering process on our posts array. For each post, the following logic will run.
